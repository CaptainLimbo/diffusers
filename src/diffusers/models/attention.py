...
class Transformer2DModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        num_self_attention: Optional[int] = 1,
    ):
        super().__init__()
...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn1 = CrossAttention(
            ...
            else None,  # normally the first attn block is a self-attn block, but if 0 self attn is specified, only
        ...
        self.attn2 = CrossAttention(
            query_dim=dim,
            cross_attention_dim=cross_attention_dim
            if num_self_attention != 2  # except the case where 2 self-attns is specified, this should be a cross-attn
            else None,
            heads=num_attention_heads,
            dim_head=attention_head_dim,
            dropout=dropout,
    ...
    def forward(self, hidden_states, context=None, timestep=None):
        ...
        hidden_states = (
            self.attn1(
                norm_hidden_states,
                context=context if self.num_self_attention == 0 else None,
            )
            + hidden_states
        )
        ...
        hidden_states = (
            self.attn2(
                norm_hidden_states,
                context=context if self.num_self_attention != 2 else None,
            )
            + hidden_states
        )
